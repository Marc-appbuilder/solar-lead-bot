...
/**
 * Resolves a clientId to its config. Mirrors Vaughan's and Bloom's
 * resolveClient() exactly.
 *
 * Step 1 always wins: every hand-configured client in lib/clients.ts is
 * returned immediately, without ever querying Supabase. 'solar-demo' and
 * 'landing-demo''s behaviour is completely unchanged from before this
 * resolver existed.
 *
 ...
 * provisioned_via = 'chatacus-v1', so it can only ever match a row
 * created by the provisioning flow — never an existing hardcoded client,
 * and never the existing inert 'solar-demo' Supabase row (which has no
 * provisioned_via set).
 *
 * If neither step finds anything, falls back to 'solar-demo', matching
